...
// --- BROWSER LAUNCHER ЗА RENDER / LOCAL ---
    ...
    try {
        ...
        if (searchQuery) {
            ...
            dramas = rawResults.filter(item => {
                ...
                if (!itemTitle.includes(cleanQuery)) return false;
    ...
    try {
        ...
        page.on("request", req => {
            ...
            if (url.includes(".m3u8") && !streamUrl) {
                ...
                req.abort(); // Спираме тежките ресурси
            } else {
                req.continue();
            }
        });
        ...
        page.goto(targetUrl, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
...
builder.defineCatalogHandler(async (args) => {
    ...
    const search = args.extra?.search || null;
...
console.log(`🚀 Add-on сървърът работи на порт ${port}`);